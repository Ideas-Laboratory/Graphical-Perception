...
for i in range(len(data_detail_names)):
    ...
    for j in np.arange(0.05,1,0.15):
        ...
        new_dict['TitlePaddingLeft']=float(j)
        fp = open(dataset_name + '_title_test/detail/testdata_detail_{}.json'.format(i),'w')
        json.dump(new_dict,fp,indent=4)
        fp.close()
        # print(new_dict)
    ...
    # if not os.path.isdir(dataset_name + '_bgColorB_test'):
    #     os.mkdir(dataset_name + '_bgColorB_test')
    # if not os.path.isdir(dataset_name + '_bgColorB_test/' + 'detail' ):
    #     os.mkdir(dataset_name + '_bgColorB_test/' + 'detail')
    ...
    # fp = open(dataset_name + '_bgColorB_test/detail/testdata_detail_{}.json'.format(i),'w')
    # json.dump(new_dict,fp,indent=4)
    # fp.close()

    # new_dict1['param'] = '{$include}' + ' detail/testdata_detail_{}.json'.format(i)
    # fp = open('{}_bgColorB_test/testdata_{}.json'.format(dataset_name,i),'w')
    # json.dump(new_dict1,fp,indent=4)
    # fp.close()
    ...
    #     new_dict1 = copy.deepcopy(b)
    #     new_dict['train']=False
    #     new_dict['bgcolorB_perturbation']=int(j)
