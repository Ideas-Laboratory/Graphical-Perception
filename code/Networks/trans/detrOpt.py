...
class DETROpt(nn.Module):
    ...
    def forward(self, samples: NestedTensor):
        ...
        if feature_useful.shape[0]==0:
            ...
            for batch_index in range(batch_size): # I do not know how to optimize this loop...
                v = outputs_count[batch_index].cpu().item()
                if v==0:
                    continue
                endPos = index+v-1
                feature_part = torch.cat([feature_useful[endPos].unsqueeze(0),feature_useful[index:endPos]],dim=0)
                feature_stack = torch.cat([feature_stack,feature_part],dim=0)
                index=endPos+1
        feature_final = torch.cat([feature_useful,feature_stack],dim=1) #[?*2M]
        out={}
...
class SetCriterion(nn.Module):
    ...
    def loss_result(self, outputs, target_result, output_mask, output_count):
        # outputs: [?]
        # outputs_class: [B*F*T]
        # target_result: [B*F] (Padding to F)
        # output_mask: [B*F] (bool)
        if outputs.shape[0]>0:
            target_mask = target_result.gt(0) #[B*F] bool
        ...
        #now, compute constrain loss
            constrain_loss = torch.tensor([0.0]).to(self.lamda_1)
            if outputs.shape[0]>1:
                index = 0
                multv=torch.Tensor([1]*batch_size).to(batch_size_tensor)
                if batch_size==1:
                    multv=multv.unsqueeze(0)
                mult_target = multv.clone()
                for batch_index in range(batch_size): # I do not know how to optimize this loop...
                    v = output_count[batch_index].cpu().item()
                    endPos = index+v
                    for i in range(index,endPos):
                        multv[batch_index]*=outputs[i]
                    index=endPos
                constrain_loss = self.mse_loss_avg(multv,mult_target)*self.lamda_1

            total_loss = normal_loss+constrain_loss
            return total_loss, constrain_loss, normal_loss
        else:
            return torch.Tensor([0]).to(target_result), torch.Tensor([0]).to(target_result), torch.Tensor([0]).to(target_result)
    
    def get_loss(self, outputs, targets):
        # TODO: modify
        loss_map={}
        loss_map["label"] = self.loss_classify(outputs["pred_class"],targets["label"])
        loss_map["r"], loss_map["r_mult"], loss_map["r_comp"] = self.loss_result(outputs["pred_v"],targets["ratio"],outputs['mask'],outputs["pred_count"])
        return loss_map
