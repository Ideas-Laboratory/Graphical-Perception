...
def add_noise(img,sigma=10,kind='normal'):
    i_shape = img.shape
    sigma = float(sigma)
...
def add_pattern(img, marked_bar_area, kind='rand'):
    if kind == 'rand':
        ...
        letters_num = np.random.randint(0, 10)
...
def add_title(img):
    ...
    first=['i','he','she','it']
    second=['do','play','choose','decide']
    third=['rose','cake','people','basketball']
    one=first[random.randint(0,len(first)-1)]
    two=second[random.randint(0,len(second)-1)]
    three=third[random.randint(0,len(third)-1)]
    title=one+" "+two+" "+three
    cv2.putText(img, title, (0,10), font, 0.3, (0, 0, 0), 1)
    return img
